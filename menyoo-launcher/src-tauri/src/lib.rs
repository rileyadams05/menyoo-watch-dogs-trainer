...
use windows::Win32::System::Threading::{CreateRemoteThread, GetExitCodeThread, OpenProcess, WaitForSingleObject, PROCESS_ACCESS_RIGHTS, PROCESS_CREATE_THREAD, PROCESS_QUERY_INFORMATION, PROCESS_QUERY_LIMITED_INFORMATION, PROCESS_VM_OPERATION, PROCESS_VM_READ, PROCESS_VM_WRITE};
use windows::Win32::UI::Input::KeyboardAndMouse::{GetKeyNameTextW, MapVirtualKeyW, MAPVK_VK_TO_VSC, VK_DELETE, VK_END, VK_HOME, VK_INSERT, VK_LEFT, VK_NEXT, VK_PRIOR, VK_RIGHT, VK_UP, VK_DOWN};
...
impl Paths {
    fn discover() -> Result<Self, String> {
        ...
        let release_dir = exe.parent().ok_or_else(|| "Failed to resolve executable directory".to_string())?.to_path_buf();
        ...
        if let Err(err) = fs::create_dir_all(&config_base) {
            return Err(format!("Failed to create config directory {:?}: {}", config_base, err));
        }
...
const CHEAT_DEFINITIONS: &[CheatDefinition] = &[
    CheatDefinition { id: "godmode", name: "God Mode", category: "Player", safe: true },
    CheatDefinition { id: "godmode", name: "Infinite Health", category: "Player", safe: true },
    CheatDefinition { id: "inffocus", name: "Infinite Focus", category: "Player", safe: true },
    CheatDefinition { id: "infbattery", name: "Infinite Battery", category: "Player", safe: true },
    CheatDefinition { id: "infskillpts", name: "Infinite Skill Points", category: "Player", safe: false },
    CheatDefinition { id: "infmoney", name: "Infinite Money", category: "Player", safe: false },
    CheatDefinition { id: "infxp", name: "Infinite XP", category: "Player", safe: false },
    CheatDefinition { id: "lockrep", name: "Reputation", category: "Player", safe: false },
    CheatDefinition { id: "notoriety", name: "Notoriety", category: "Player", safe: false },
    CheatDefinition { id: "lockammo", name: "Infinite Ammo", category: "Weapons / Items", safe: true },
    CheatDefinition { id: "lockammo", name: "No Reload", category: "Weapons / Items", safe: true },
    CheatDefinition { id: "lockcraft", name: "Infinite Craft Materials", category: "Weapons / Items", safe: true },
    CheatDefinition { id: "refillwheel", name: "Infinite Items", category: "Weapons / Items", safe: false },
    CheatDefinition { id: "stealth", name: "Invisible", category: "Stealth / Police", safe: false },
    CheatDefinition { id: "stealth", name: "Undetectable", category: "Stealth / Police", safe: false },
    CheatDefinition { id: "clearheat", name: "Wanted Level Control", category: "Stealth / Police", safe: true },
    CheatDefinition { id: "clearheat", name: "Police Radar / Heat Control", category: "Stealth / Police", safe: true },
    CheatDefinition { id: "noclip", name: "No Clip / Free Roam", category: "World / Movement", safe: false },
    CheatDefinition { id: "oneteleport", name: "Teleport To Waypoint", category: "World / Movement", safe: false },
    CheatDefinition { id: "savecords", name: "Save Coordinates", category: "World / Movement", safe: false },
    CheatDefinition { id: "restorecords", name: "Restore Coordinates", category: "World / Movement", safe: false },
    CheatDefinition { id: "settime", name: "Time Of Day", category: "World / Movement", safe: false },
    CheatDefinition { id: "overidefov", name: "FOV / Camera Distance", category: "World / Movement", safe: false },
    CheatDefinition { id: "onehitcar", name: "Car Health", category: "Vehicles", safe: false },
    CheatDefinition { id: "onehitcar", name: "One Hit Destroy Vehicles", category: "Vehicles", safe: false },
    CheatDefinition { id: "savecords", name: "Vehicle Coordinates", category: "Vehicles", safe: false },
    CheatDefinition { id: "infhacktime", name: "Infinite Hacking Time", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "spidertimer", name: "Spider Tank Cheats", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "cashrunlock", name: "Cash Run", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "nvznlock", name: "NVZN", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "madnesstimer", name: "Madness Timer", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "ctosstop", name: "ctOS Timer", category: "Mini Games / Digital Trips", safe: true },
    CheatDefinition { id: "infskillpts", name: "Unlock All", category: "Misc", safe: false },
    CheatDefinition { id: "onehitcar", name: "One Hit Kill", category: "Misc", safe: false },
    CheatDefinition { id: "poker1", name: "Poker Money", category: "Misc", safe: false },
];
...
fn monitor_tick(app: &AppHandle, shared: &AppState) -> Result<(), String> {
    ...
    let mut guard = shared.inner.lock().map_err(|_| "State poisoned".to_string())?;
    ...
    match pid {
        Some(process_id) => {
            ...
            if guard.pending_pid != Some(process_id) {
                ...
                guard.status.status_message = Some("Waiting for Watch Dogs to stabilise (15s) before injection.".into());
            ...
            if let Some(rx) = guard.injection_rx.as_ref() {
                if let Ok(result) = rx.try_recv() {
                    ...
                    if result.success {
                        ...
                        guard.status.status_message = Some("Verifying diagnostic DLL load (30s).".into());
                        guard.status.last_error = None;
                        append_loader_log(&shared.paths, "Injection completed. Beginning 30-second diagnostic verification.");
                    } else {
                        guard.status.trainer_stage = TrainerStage::Failed;
                        let msg = result.message.unwrap_or_else(|| "Injection failed. See loader.log for details.".into());
                        guard.status.status_message = Some(msg.clone());
                        guard.status.last_error = Some(msg.clone());
                        guard.last_error = Some(msg.clone());
                        append_loader_log(&shared.paths, &format!("Injection failed for PID {}: {}", process_id, msg));
                    }
                }
            }
            ...
            match guard.status.trainer_stage {
                TrainerStage::Waiting => {
                    if let Some(start) = guard.wait_started_at {
                        ...
                        if remaining > 0 {
                            guard.status.status_message = Some(format!("Injecting in {}s...", remaining));
                        }
                ...
                TrainerStage::Injecting => {
                    if let Some(start) = guard.injection_started_at {
                        if now.saturating_duration_since(start).as_secs() > INJECTION_TIMEOUT_SECONDS {
                            guard.status.trainer_stage = TrainerStage::Failed;
                            ...
                            guard.injection_started_at = None;
                            guard.attempted_pids.insert(process_id);
                            append_loader_log(&shared.paths, &format!("Injection timed out for PID {}", process_id));
                        } else {
                ...
                TrainerStage::Verifying => {
                    if let Some(start) = guard.verify_started_at {
                        ...
                        if !is_process_alive(process_id) {
                            ...
                            guard.verify_started_at = None;
                            guard.attempted_pids.insert(process_id);
                            append_loader_log(&shared.paths, "Diagnostic verification failed: process terminated early.");
                        } else if elapsed.as_secs() >= VERIFY_SECONDS {
                            guard.status.trainer_stage = TrainerStage::DiagnosticLoaded;
                            guard.status.status_message = Some("Diagnostic DLL loaded successfully.".into());
                            guard.status.verifying_seconds_remaining = None;
                            guard.verify_started_at = None;
                            guard.last_error = None;
                            append_loader_log(&shared.paths, "Diagnostic DLL remained loaded after 30 seconds.");
                        } else {
                            guard.status.verifying_seconds_remaining = Some(VERIFY_SECONDS - elapsed.as_secs());
        ...
        None => {
            ...
            if let Some((pid_lost, grace_start)) = guard.grace_until {
                if now.saturating_duration_since(grace_start).as_secs() >= GRACE_SECONDS {
                    ...
                    guard.grace_until = None;
                    guard.last_error = None;
                    append_loader_log(&shared.paths, &format!("watch_dogs.exe (PID {}) closed. Returning to idle state.", pid_lost));
                }
            } else if guard.status.game_on {
                guard.grace_until = Some((guard.status.process_id, now));
                append_loader_log(&shared.paths, &format!(
    ...
    let should_emit = guard.last_serialized.as_ref().map(|prev| prev != &serialized).unwrap_or(true);
...
fn perform_injection(pid: u32, dll_path: &Path) -> InjectionOutcome {
    match inject_trainer(pid, dll_path) {
        Ok(_) => InjectionOutcome { success: true, error_code: 0, message: None, pid },
...
fn inject_trainer(pid: u32, dll_path: &Path) -> Result<(), InjectionError> {
    unsafe {
        ...
        let remote = VirtualAllocEx(process, None, byte_len, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        if remote.is_null() {
        ...
        match WriteProcessMemory(process, remote, dll_wide.as_ptr() as *const c_void, byte_len, None) {
            Ok(_) => {}
            Err(e) => {
        ...
        let load_lib = GetProcAddress(kernel32, PCSTR(proc_name.as_ptr()))
            .ok_or_else(|| {
                let _ = VirtualFreeEx(process, remote, 0, MEM_RELEASE);
                let _ = CloseHandle(process);
                InjectionError {
...
fn ensure_log_files(paths: &Paths) {
    for log in [&paths.loader_log, &paths.trainer_log] {
        if OpenOptions::new().append(true).create(true).open(log).is_ok() {
            continue;
        }
    }
}
...
fn append_loader_log(paths: &Paths, message: &str) {
    if let Ok(mut file) = OpenOptions::new().append(true).create(true).open(&paths.loader_log) {
...
fn enforce_diagnostic_restrictions(paths: &Paths, config: &mut TrainerConfig) -> Result<(), String> {
    ...
    if changed {
        append_loader_log(paths, "Diagnostics: forced all cheats OFF because diagnosticMode=true.");
...
fn vk_to_label(vk: u32) -> Option<String> {
    ...
    unsafe {
        ...
        let extended_keys = [VK_INSERT.0, VK_DELETE.0, VK_HOME.0, VK_END.0, VK_PRIOR.0, VK_NEXT.0, VK_LEFT.0, VK_RIGHT.0, VK_UP.0, VK_DOWN.0];
...
impl TrainerConfig {
    fn load(path: &Path) -> Self {
        let (raw, modified) = match fs::read_to_string(path) {
            Ok(content) => {
                let value = serde_json::from_str::<serde_json::Value>(&content).unwrap_or_else(|_| serde_json::Value::Object(Default::default()));
    ...
    fn ensure_object<'a>(map: &'a mut serde_json::Map<String, serde_json::Value>, key: &str) -> &'a mut serde_json::Map<String, serde_json::Value> {
        map.entry(key.to_string()).or_insert_with(|| serde_json::Value::Object(Default::default()));
    ...
    fn ensure_defaults(&mut self) {
        let root = self.ensure_root();
        root.entry("menuHotkey").or_insert(serde_json::Value::Number((0x5B as u32).into()));
        root.entry("fov").or_insert(serde_json::Value::Number(serde_json::Number::from_f64(75.0).unwrap()));
        root.entry("timeHours").or_insert(serde_json::Value::Number(serde_json::Number::from_f64(12.0).unwrap()));
        root.entry("maxAmmo").or_insert(serde_json::Value::Number(serde_json::Number::from(9999)));
        root.entry("maxCraft").or_insert(serde_json::Value::Number(serde_json::Number::from(999)));
    }
...
fn read_log_tail(path: &Path, max_bytes: usize) -> Result<String, String> {
    ...
    let start = if size > max_bytes as u64 { size - max_bytes as u64 } else { 0 };
...
#[tauri::command]
fn set_cheat_state(app: AppHandle, state: State<AppState>, cheat_id: String, enabled: bool) -> Result<TrainerStatus, String> {
    {
        let guard = state.inner.lock().map_err(|_| "State poisoned".to_string())?;
    ...
    let mut guard = state.inner.lock().map_err(|_| "State poisoned".to_string())?;
    guard.status.cheats = build_cheat_entries(&config, guard.status.diagnostic_mode);
    guard.status.status_message = Some(format!("{} set to {}.", cheat_id, if enabled { "ON" } else { "OFF" }));
    guard.status.last_error = None;
    guard.status.timestamp = Utc::now().to_rfc3339();
    guard.last_serialized = None;
    let payload = guard.status.clone();
...
#[tauri::command]
fn set_cheat_hotkey(app: AppHandle, state: State<AppState>, cheat_name: String, hotkey_vk: u32) -> Result<TrainerStatus, String> {
    let mut config = TrainerConfig::load(&state.paths.trainer_config);
...
#[tauri::command]
fn clear_cheat_hotkey(app: AppHandle, state: State<AppState>, cheat_name: String) -> Result<TrainerStatus, String> {
    let mut config = TrainerConfig::load(&state.paths.trainer_config);
